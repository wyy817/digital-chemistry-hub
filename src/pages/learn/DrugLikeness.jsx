...
import { Ch6DrugLikenessContent, ch6DrugLikenessMeta } from '../../content/ch6-drug-likeness'
import { ch62QuickCheck } from '../../content/ch6-drug-likeness-quiz'

const PAGE_ID = 'ch6-2-drug-likeness'

const TOC = [
  { id: 's1', label: '6.2.1 什么是"类药性"？Lipinski 之后' },
  { id: 's2', label: '6.2.2 极性表面积（PSA）与渗透性' },
  { id: 's3', label: '6.2.3 结构警示与 PAINS' },
  { id: 's4', label: '6.2.4 先导优化：Matched Molecular Pairs' },
  { id: 's5', label: '6.2.5 用 ML 预测 ADMET' },
  { id: 's6', label: '6.2.6 综合案例：多属性优化（MPO）' },
]
...
export default function DrugLikeness() {
  ...
  const meta = ch6DrugLikenessMeta
    ...
    <Layout progress={progress} annotations={annotations} onDeleteAnnotation={deleteAnnotation} onUpdateAnnotation={updateAnnotation} showAnnotations wide initialSidebarCollapsed>
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch6DrugLikenessContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch62QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
