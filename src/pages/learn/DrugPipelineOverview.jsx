...
import { Ch5DrugPipelineOverviewContent, ch5DrugPipelineOverviewMeta } from '../../content/ch5-drug-pipeline-overview'
import { ch51QuickCheck } from '../../content/ch5-drug-pipeline-overview-quiz'

const PAGE_ID = 'ch5-1-drug-pipeline-overview'

const TOC = [
  { id: 's1', label: '5.1.1 传统药物发现的困境' },
  { id: 's2', label: '5.1.2 完整流程' },
  { id: 's3', label: '5.1.3 AI 的切入点' },
  { id: 's4', label: '5.1.4 化学空间挑战' },
  { id: 's5', label: '5.1.5 里程碑案例' },
  { id: 's6', label: '5.1.6 学习路线图' },
]
...
export default function DrugPipelineOverview() {
  const { user } = useAuth()
  ...
  const meta = ch5DrugPipelineOverviewMeta
      ...
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          <nav className="breadcrumb anim-fade-up anim-delay-1">
            <Link to="/">Dashboard</Link><span className="breadcrumb-sep">›</span><span>AI in Drug Discovery</span><span className="breadcrumb-sep">›</span><span>Ch5. Drug Discovery Pipeline</span><span className="breadcrumb-sep">›</span><span style={{ color: 'var(--color-text)' }}>{meta.titleZh}</span>
          </nav>
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch5DrugPipelineOverviewContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch51QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
