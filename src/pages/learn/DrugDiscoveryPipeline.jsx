...
import { Ch3DrugDiscoveryPipelineContent, ch3DrugDiscoveryPipelineMeta } from '../../content/ch3-drug-discovery-pipeline'
import { ch32QuickCheck } from '../../content/ch3-drug-discovery-pipeline-quiz'

const PAGE_ID = 'ch3-2-drug-discovery-pipeline'

const TOC = [
  { id: 's1', label: '3.2.1 药物发现总览' },
  { id: 's2', label: '3.2.2 靶点发现与验证' },
  { id: 's3', label: '3.2.3 苗头化合物发现' },
  { id: 's4', label: '3.2.4 先导化合物优化' },
  { id: 's5', label: '3.2.5 临床前与临床研究' },
  { id: 's6', label: '3.2.6 AI 在药物发现中的应用' },
]
...
export default function DrugDiscoveryPipeline() {
  const { user } = useAuth()
  ...
  const meta = ch3DrugDiscoveryPipelineMeta
      ...
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          <nav className="breadcrumb anim-fade-up anim-delay-1">
            <Link to="/">Dashboard</Link><span className="breadcrumb-sep">›</span><span>Chemistry Fundamentals</span><span className="breadcrumb-sep">›</span><span>Ch3. Drug-like Properties</span><span className="breadcrumb-sep">›</span><span style={{ color: 'var(--color-text)' }}>{meta.titleZh}</span>
          </nav>
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch3DrugDiscoveryPipelineContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch32QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
