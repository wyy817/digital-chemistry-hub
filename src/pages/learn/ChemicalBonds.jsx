...
import { Ch1ChemicalBondsContent, ch1ChemicalBondsMeta } from '../../content/ch1-chemical-bonds'
import { ch12QuickCheck } from '../../content/ch1-chemical-bonds-quiz'

const PAGE_ID = 'ch1-2-chemical-bonds'

const TOC = [
  { id: 's1', label: '1.2.1 为什么原子会键合' },
  { id: 's2', label: '1.2.2 共价键' },
  { id: 's3', label: '1.2.3 离子键' },
  { id: 's4', label: '1.2.4 氢键' },
  { id: 's5', label: '1.2.5 范德华力' },
  { id: 's6', label: '1.2.6 化学键与药物发现' },
...
export default function ChemicalBonds() {
  const { user } = useAuth()
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setInputBox(null)
    setToolbar({ x: rect.left + rect.width / 2, y: rect.top - 10 + window.scrollY, text, range: range.cloneRange() })
  }, [])

  useEffect(() => {
    const handler = (e) => {
      if (!e.target.closest('.annotation-toolbar') && !e.target.closest('.annotation-input-box')) {
        setToolbar(null); setInputBox(null); setInputNote('')
      }
    }
  ...
  function openInputBox() {
    ...
    setToolbar(null); setInputNote('')
  }

  async function saveAnnotation() {
    if (!inputBox || !user || !inputNote.trim()) return
    const { data, error } = await supabase.from('annotations').insert({
      user_id: user.id, page_id: PAGE_ID, selected_text: inputBox.text,
      note: inputNote.trim(), color: 'yellow', container_id: 'content-main',
    }).select().single()
    if (!error && data) {
      setAnnotations(prev => [...prev, data])
      try { const span = document.createElement('span'); span.className = 'annotation-highlight'; span.dataset.annotationId = data.id; inputBox.range.surroundContents(span) } catch { }
  ...
  const meta = ch1ChemicalBondsMeta
    ...
    <Layout progress={progress} annotations={annotations} onDeleteAnnotation={deleteAnnotation} onUpdateAnnotation={updateAnnotation} showAnnotations wide initialSidebarCollapsed>
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          ...
          <p className="anim-fade-up anim-delay-3" style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)', marginBottom: '1.75rem', borderLeft: '3px solid var(--color-border)', paddingLeft: '0.75rem' }}>
            💡 选中页面中的文字可以添加批注，点击右侧「批注」按钮打开批注面板。
          </p>
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4">
            <Ch1ChemicalBondsContent />
          </div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch12QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
        ...
        <div className="learn-aside anim-slide-right anim-delay-2">
          <div className="learn-toc">
            ...
            <ul className="learn-toc-list">
              {TOC.map(sec => (
                <li key={sec.id}><button className={`learn-toc-item ${activeSection === sec.id ? 'active' : ''}`} onClick={() => document.getElementById(sec.id)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>{sec.label}</button></li>
              ))}
            </ul>
          </div>
