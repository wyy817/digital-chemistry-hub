...
import { Ch5HitLeadCandidateContent, ch5HitLeadCandidateMeta } from '../../content/ch5-hit-lead-candidate'
import { ch53QuickCheck } from '../../content/ch5-hit-lead-candidate-quiz'

const PAGE_ID = 'ch5-3-hit-lead-candidate'

const TOC = [
  { id: 's1', label: '5.3.1 三个阶段的定义' },
  { id: 's2', label: '5.3.2 苗头化合物发现 (HTS)' },
  { id: 's3', label: '5.3.3 Hit-to-Lead 优化' },
  { id: 's4', label: '5.3.4 先导化合物优化 (LO)' },
  { id: 's5', label: '5.3.5 AI 加速优化' },
  { id: 's6', label: '5.3.6 多参数优化 (MPO)' },
]
...
export default function HitLeadCandidate() {
  const { user } = useAuth()
  ...
  const meta = ch5HitLeadCandidateMeta
    ...
    <Layout progress={progress} annotations={annotations} onDeleteAnnotation={deleteAnnotation} onUpdateAnnotation={updateAnnotation} showAnnotations wide initialSidebarCollapsed>
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch5HitLeadCandidateContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch53QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
