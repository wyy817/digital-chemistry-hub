...
import { Ch6AdmetDeepDiveContent, ch6AdmetDeepDiveMeta } from '../../content/ch6-admet-deep-dive'
import { ch61QuickCheck } from '../../content/ch6-admet-deep-dive-quiz'

const PAGE_ID = 'ch6-1-admet-deep-dive'

const TOC = [
  { id: 's1', label: '6.1.1 ADMET 全景：为什么 90% 失败？' },
  { id: 's2', label: '6.1.2 Absorption：吸收与生物利用度' },
  { id: 's3', label: '6.1.3 Distribution：分布与血脑屏障' },
  { id: 's4', label: '6.1.4 Metabolism：CYP 酶与代谢稳定性' },
  { id: 's5', label: '6.1.5 Excretion & Toxicity' },
  { id: 's6', label: '6.1.6 Lipinski 五规则与 RDKit 筛选' },
]
...
export default function AdmetDeepDive() {
  const { user } = useAuth()
  ...
  const meta = ch6AdmetDeepDiveMeta
      ...
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          <nav className="breadcrumb anim-fade-up anim-delay-1">
            <Link to="/">Dashboard</Link><span className="breadcrumb-sep">›</span><span>AI in Drug Discovery</span><span className="breadcrumb-sep">›</span><span>Ch6. ADMET & Drug-likeness</span><span className="breadcrumb-sep">›</span><span style={{ color: 'var(--color-text)' }}>{meta.titleZh}</span>
          </nav>
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch6AdmetDeepDiveContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch61QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
