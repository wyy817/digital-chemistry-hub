...
import { Ch4QsarContent, ch4QsarMeta } from '../../content/ch4-qsar'
import { ch43QuickCheck } from '../../content/ch4-qsar-quiz'

const PAGE_ID = 'ch4-3-qsar'

const TOC = [
  { id: 's1', label: '4.3.1 什么是 QSAR？' },
  { id: 's2', label: '4.3.2 数据集准备' },
  { id: 's3', label: '4.3.3 构建第一个 QSAR 模型' },
  { id: 's4', label: '4.3.4 模型评估与交叉验证' },
  { id: 's5', label: '4.3.5 模型解释性' },
  { id: 's6', label: '4.3.6 QSAR 的局限性与最佳实践' },
]
...
export default function QsarModeling() {
  const { user } = useAuth()
    ...
    <Layout progress={progress} annotations={annotations} onDeleteAnnotation={deleteAnnotation} onUpdateAnnotation={updateAnnotation} showAnnotations wide initialSidebarCollapsed>
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch4QsarContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch43QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
