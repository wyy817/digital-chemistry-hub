...
import { Ch4RdkitBasicsContent, ch4RdkitBasicsMeta } from '../../content/ch4-rdkit-basics'
import { ch41QuickCheck } from '../../content/ch4-rdkit-basics-quiz'

const PAGE_ID = 'ch4-1-rdkit-basics'

const TOC = [
  { id: 's1', label: '4.1.1 什么是 RDKit？' },
  { id: 's2', label: '4.1.2 分子读取与基本操作' },
  { id: 's3', label: '4.1.3 分子可视化' },
  { id: 's4', label: '4.1.4 提取分子基本信息' },
  { id: 's5', label: '4.1.5 计算分子描述符' },
  { id: 's6', label: '4.1.6 子结构搜索' },
]
...
export default function RdkitBasics() {
  const { user } = useAuth()
      ...
      <div className="learn-two-col anim-fade-in">
        <div className="learn-content-main">
          <nav className="breadcrumb anim-fade-up anim-delay-1">
            <Link to="/">Dashboard</Link><span className="breadcrumb-sep">›</span><span>Cheminformatics</span><span className="breadcrumb-sep">›</span><span>Ch4. RDKit & Cheminformatics</span><span className="breadcrumb-sep">›</span><span style={{ color: 'var(--color-text)' }}>{meta.titleZh}</span>
          </nav>
          ...
          <div id="content-main" ref={contentRef} onMouseUp={handleMouseUp} className="anim-fade-up anim-delay-4"><Ch4RdkitBasicsContent /></div>
          <div style={{ marginTop: '2.5rem' }}>
            ...
            {showQuiz && <QuickCheck quiz={ch41QuickCheck} onComplete={({ passed }) => { if (passed) setIsCompleted(true) }} />}
          </div>
